...
public class WorldContactListener implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        //projectiles when hitting walls
            ...
            io.github.devsimulator.entities.SandProjectile p = (io.github.devsimulator.entities.SandProjectile) dataA;
            // If it's NOT lava, destroy it. If it IS lava, let it live so it can roll!
            if (!"LAVA".equals(p.element)) {
                p.isDestroyed = true;
            }
        }
        else if (dataB instanceof io.github.devsimulator.entities.SandProjectile && "GROUND".equals(dataA)) {
            io.github.devsimulator.entities.SandProjectile p = (io.github.devsimulator.entities.SandProjectile) dataB;
            if (!"LAVA".equals(p.element)) {
                p.isDestroyed = true;
            }
        }
        else if (dataA instanceof io.github.devsimulator.entities.SandProjectile && "PLAYER".equals(dataB)) {
            handlePlayerProjectileHit((io.github.devsimulator.entities.SandProjectile) dataA, playerInstance);
        } else if (dataB instanceof io.github.devsimulator.entities.SandProjectile && "PLAYER".equals(dataA)) {
            handlePlayerProjectileHit((io.github.devsimulator.entities.SandProjectile) dataB, playerInstance);
        }
    ...
    private void handlePlayerProjectileHit(io.github.devsimulator.entities.SandProjectile proj, Player player) {
        if (proj.isDestroyed || player == null) return;

        // Destroy projectile so it doesn't hit twice
        proj.isDestroyed = true;

        // Use the Mega flag you set in LavaSlime to deal more damage
        float dmg = proj.isMega ? 30f : 10f;

        // Calculate knockback direction based on where the projectile is relative to player
        float knockbackDir = (proj.b2body.getPosition().x < player.b2body.getPosition().x) ? 5f : -5f;

        // Call your player's damage method
        player.takeDamage(dmg, knockbackDir, null);
    }
