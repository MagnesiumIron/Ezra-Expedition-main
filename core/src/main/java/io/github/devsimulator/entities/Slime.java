...
public class Slime extends Enemy {
    ...
    public Slime(World world, Player player, float x, float y) {
        ...
        jumpSound = Gdx.audio.newSound(Gdx.files.internal("slime_sounds/normal_slime.wav"));
    ...
    public void update(float dt, SandManager sandMgr) {
        ...
        if (isGrounded) {
            ...
            if (timer >= jumpInterval && isGrounded) {
                ...
                float randomVolume = 0.4f + (float)Math.random() * 0.2f;
                long id = jumpSound.play(randomVolume);
                jumpSound.setPitch(id, randomPitch);
                isJumping = true;
                timer = 0;
            }
        }
    ...
    public void draw(SpriteBatch batch) {
        ...
        if (isJumping) currentFrame = slimeAnimation.getKeyFrame(0.35f);
        else if (isSquishing) currentFrame = slimeAnimation.getKeyFrame(0.16f);
        else currentFrame = slimeAnimation.getKeyFrame(0.0f);
        ...
        batch.draw(currentFrame,
            ...
            (b2body.getPosition().y * Main.PPM) - 31.25f,
            50f, 50f);
    }
